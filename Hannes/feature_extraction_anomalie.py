...
for i in range(4):  # for Schleife auf Bilder aus verschiedenen Ordnern zuzugreifen
    if i == 0:
        input_dir = "photos_reduced/Zwiebel_reduced"
        ...
        input_dir = "photos_reduced/Karotte_reduced"
        ...
        input_dir = "photos_reduced/Kartoffel_reduced"
        output_dir = "photos_reduced/Kartoffel_reduced"
        files = os.listdir(input_dir)
        count = 2*max_files_global+1
        max_files = max_files_global*3
    if i == 3:
        input_dir = "photos_reduced_anomalie"
        output_dir = "photos_reduced_anomalie"
        files = os.listdir(input_dir)
        count = 3*max_files_global+1
        max_files = max_files_global*4
...
df.to_csv("output_anomalie.csv", sep=",")
